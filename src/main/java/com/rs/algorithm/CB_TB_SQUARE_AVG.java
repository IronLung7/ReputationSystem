 ...
 */
public class CB_TB_SQUARE_AVG implements BasicAlgorithm {

    Float beta;

    public CB_TB_SQUARE_AVG(){    beta = new Float(0.98);}

    public CB_TB_SQUARE_AVG(Float beta){
        this.beta=beta;
    }
                ...
                for (Rating rating : ratings) {
                    ...
                    if (itemTopics.size() != 0) {
                        formula0 += itemTopics.get(0).getLevel()
                                * Math.abs(rating.getScore()- rating.getItem().getE_score())
                                * Math.abs(rating.getScore()- rating.getItem().getE_score());
                        formula1 += itemTopics.get(0).getLevel();
                    }
                }
                if (formula1 != 0) {
                    reputation = 1 - (lamda/2) * formula0 / formula1;
                }
